# Intake Record Draft — `scene_00_intro_entry` Candidate 01

## 文档定位
本文件是 `scene_00_intro_entry` 的第一条 candidate evidence intake 记录草案。
...
## 当前约束来源
本记录必须同时服从以下上游约束：
- `chapter_00_intro` 的核心论断是：本项目的综合场景是受证据约束的教学容器，不是默认意义上的单一真实完整遗址。
- 本章默认 `truth_mode_default` 为 `B_composite_teaching_scene`，只承担观看入口与规则入口，不承担知识高潮。
- 本章禁止用人物、仪式、服饰、神怪场景代替真实性声明。
- `B_composite_teaching_scene` 不能冒充单一真实完整遗址；`E2_unresolved` 不得进入最终下游产物。
- 当前 `scene_00_intro_entry` 仍挂在 `TBD_CASE_ANCHOR_INTRO_01`、`SRC_TBD_INTRO_01` 与 `E2_unresolved` 上。

---

## A. Intake meta
- `intake_id`: `INTAKE_SCENE00_INTRO_CANDIDATE_01_DRAFT`
...
- `linked_scene_intake`: `research/intake_scene_00_intro_entry_draft.md`
- `linked_collection_brief`: `research/scene_00_intro_candidate_collection_brief.md`

## B. Candidate target type
- `candidate_role`: `primary_anchor_candidate`
- `candidate_goal`: 为入口 scene 提供第一条主锚点候选与第一组来源候选
- `why_this_candidate_exists`:
  - 当前 `scene_00_intro_entry` 不能继续只靠占位 source 与占位 anchor 维持；
  - 需要先形成第一条主锚点候选记录，才能进入正式 backfill 判断；
  - 该候选必须优先支撑“观看边界与真实性说明”，不是支撑地标感、宣传感或氛围震撼。
...
- `why_this_case_fits_this_scene`:
  - 需能支撑入口层的观看边界与最小真实性说明；
  - 需能服务“综合教学场景而非单一完整遗址”的入口逻辑；
  - 需能在综合教学场景中保留阈限与进入关系，而不是依赖单一地标感成立。
- `why_this_case_might_fail`:
  - 若它只能支撑“像古建入口”的氛围而不能支撑观看边界，则不应做 primary anchor；
  - 若它的地标性强到会把 scene 拉向单一完整遗址误读，则不适合当前主 scene；
  - 若它会诱导视觉团队追求“入口奇观”而不是“边界可读”，则应降级为 support 或弃用。
...
- `supports_which_claim_exactly`:
  - 至少需支持以下之一：
    1. 综合教学场景的披露义务；
    2. 入口场景不能伪装成单一真实完整遗址；
    3. 哪些入口视觉语法可进入 scene，哪些不能写成实景证明。
- `what_this_source_does_not_support`:
  - 不自动支持单一真实完整遗址证明；
  - 不自动支持章节主体知识高潮；
  - 不自动支持任何人物、仪式或表演性历史场景。
...
- `false_documentary_risk`: `high`
- `promo-overread_risk`: `high`
- `pageant-overread_risk`: `medium`
- `reason_for_high_risk`:
  - 入口章天然容易被视觉团队拍成旅游宣传入口或单一遗址入口；
  - 在真实来源未补齐前，任何看起来很强的入口画面都可能越界成伪证据。

## F. Review questions for this candidate
1. 这条候选到底支持的是哪条观看边界，而不是哪种“震撼入口感”？
2. 它是否足以成为 primary anchor，还是只能做 support？
3. 它是否会诱导我们把综合教学场景拍成单一完整遗址入口？
